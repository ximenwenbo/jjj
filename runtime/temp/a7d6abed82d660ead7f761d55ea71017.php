<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:60:"F:\www\jjj\public/../application/admin\view\user\xiugai.html";i:1536307081;s:47:"F:\www\jjj\application\admin\view\pub\head.html";i:1536568864;s:47:"F:\www\jjj\application\admin\view\pub\foot.html";i:1532090900;s:53:"F:\www\jjj\application\admin\view\user\js_xiugai.html";i:1536310698;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
	...
	<form action="" method="post" class="form form-horizontal" id="add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $info['user_name']; ?>" placeholder="" id="username" name="user_name">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal">
				<div class="radio-box">
					<input name="sex" type="radio" id="sex-1" value="男"  <?php if($info['sex'] == '男'): ?> checked
					<?php endif; ?>>
					<label for="sex-1">男</label>
				</div>
				<div class="radio-box">
					<input type="radio" id="sex-2"  value="女" name="sex" <?php if($info['sex'] == '女'): ?> checked<?php endif; ?>>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $info['age']; ?>" placeholder="" id="mobile" name="age">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>国籍：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $info['state']; ?>" name="state" id="email">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $info['hand']; ?>" placeholder="" id="" name="hand">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>直横拍：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $info['bat']; ?>" placeholder=""  name="bat">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>打法：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $info['play']; ?>" placeholder=""  name="play">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<div id="images"></div>
				<input type="hidden" class="input-text" value="<?php echo substr($info['image'],1); ?>"   name="image" id="image">
			</div>
		</div>
		<div class="row cl">
...
<script type="text/javascript">
    $(function(){
        new HHuploadify({
           ...
           onInit:function(){
               //获得goods_logo图片的路径名 并显示出来
               var image = $("#image").val();
               //判断当前商品logo图片 并显示出来
               if(images != ""){
                   this.options.files = [{path:image}];
               }
    ...
    $(function(){
        $('#add').submit(function(evt){
            ...
            $.ajax({
                ...
                url:'<?php echo url("user/xiugai","",false); ?>',    // admin/users/tianjia
                data:shuju,
                dataType:'json',
                type:'post',
                success:function(msg){
                    ...
                    if(msg.status=='success'){
                        layer.alert('修改运动员成功',{icon:6},function(){
                            //下述① 和 ②执行执行顺序要求
                            //① 在"父级"页面把新添加商品刷新出来
                        ...
                        //layer_close();
                        //});
                        layer.alert('修改运动员信息失败【'+msg.errorinfo+'】',{icon:5});
                    }
                }
            });
        });
